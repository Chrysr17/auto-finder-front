    ...
    <main className="min-h-screen p-6">
      <header className="flex items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm opacity-80">Catálogo desde tu Gateway</p>
        </div>
        <LogoutButton />
      </header>
              ...
              >
                <div className="aspect-[16/9] w-full overflow-hidden rounded-xl border border-white/10 bg-black/20">
                  {a.imagenPortadaUrl ? (
                    ...
                    <img src={a.imagenPortadaUrl} alt={`Auto ${a.id}`} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full items-center justify-center text-sm opacity-70">Sin imagen</div>
                  )}
                </div>
